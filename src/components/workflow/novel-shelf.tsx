    ...
    <div className="rounded-3xl border border-slate-900/10 bg-white/80 p-5 shadow-xs backdrop-blur-md space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-200/80">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              共已工业化生产 {projects.length} 部小说全案，各书参数与人设独立绑定
            </p>
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={onCreateNew}
          className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 px-3.5 py-1.5 text-xs font-semibold text-white hover:opacity-95 shadow-2xs transition-opacity cursor-pointer"
        >
      ...
      {projects.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center space-y-3 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
          ...
          <div className="space-y-1 max-w-sm">
            ...
            <p className="text-[11px] text-muted-foreground">
              在下方输入您的初始灵感并点击「启动工业化流水线」，生成第一章时将自动生成封面并入库书架。
            </p>
          </div>
        </div>
      )}
